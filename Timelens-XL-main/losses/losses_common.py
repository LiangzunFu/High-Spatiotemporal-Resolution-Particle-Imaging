...
class maskpsnr():
    def __init__(self, loss_dict):
        ...
        self.as_loss = loss_dict.as_loss
        self.weight = loss_dict.weight
        self.loss_item = torch.nn.MSELoss(reduction='mean')

    def masked_mse(self, x, y, mask):
        """计算 mask 区域的 MSE，mask=1 的地方有效"""
    ...
    def forward(self, x, y, mask, data_range=1.):

        with torch.no_grad():
            mask_ratio = (mask == 1).sum() / mask.numel()
            mse_event = self.masked_mse(x, y, mask)
            if mse_event is None:
                psnr_event = None    # mask中没有事件
            else:
                psnr_event = 10 * torch.log10(data_range**2 / (mse_event + 1e-14))

            non_mask = (1 - mask)
            mse_non = self.masked_mse(x, y, non_mask)
            if mse_non is None:
                psnr_non_event = None
            else:
                psnr_non_event = 10 * torch.log10(data_range**2 / (mse_non + 1e-14))

            return {
                "mask_ratio": mask_ratio,
                "psnr_event": psnr_event,
                "psnr_non_event": psnr_non_event
            }

def charbonnier_loss(pred, target, eps=1e-6):
    return torch.mean(torch.sqrt((pred - target)**2 + eps))

def charbonnier_loss_with_mask(pred, target, mask, eps=1e-6):

    loss = torch.sqrt((pred - target) ** 2 + eps)
    mask = mask.unsqueeze(2)
    weight = mask * 50.0 + (1 - mask) * 1
    loss = loss * weight

    return torch.mean(loss)

@LOSS_REGISTRY.register()
class MaskCharbonier():
    def __init__(self, loss_dict):
        self.loss_dict = loss_dict
        self.as_loss = loss_dict.as_loss
        self.weight = loss_dict.weight
        self.loss_item = charbonnier_loss_with_mask

    def forward(self, x, y, mask):
        if not self.as_loss:
            with torch.no_grad():
                loss = self.loss_item(x, y, mask)*self.weight
        else:
            loss = self.loss_item(x, y, mask)*self.weight
        return loss
...
class Charbonier():
    ...
    def forward(self, x, y):
        if not self.as_loss:
            with torch.no_grad():
                loss = self.loss_item(x, y)*self.weight
        else:
            loss = self.loss_item(x, y)*self.weight
        return loss
