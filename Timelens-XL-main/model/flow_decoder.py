...
# 根据原图和光流图进行图像Wrapping，u是X轴上的像素位移，v是Y轴上的像素位移
...
class FlowDecoder(nn.Module):
	def __init__(self, in_channel, base_channel, conv_base_channel): # 384, 128, 128
		super().__init__()
		# 384 -> 128
...
class MaskNet(nn.Module):
	def __init__(self, in_channel, base_channel):
		...
		self.channel_squeeze0 = nn.Conv2d(base_channel, base_channel//4, 1, 1, 0, bias=True)
		self.channel_squeeze1 = nn.Conv2d(base_channel, base_channel//4, 1, 1, 0, bias=True)
		self.refine_blocks = ResBlockIF(3 + base_channel, base_channel//2)
		self.conv_out = nn.Conv2d(base_channel//2, 3, 1, 1, bias=True)
