...
import numpy as np
from model.Forward_warp import ForwardWarp
import os
from tools.flow_vis import flow_to_color
...
class BaseModel(Module):
    ...
    def _init_metrics(self, metrics):
        ...
        if 'val_maskpsnr' in self.metrics_record.keys():
            self.metrics_record['val_maskpsnr_mask_ratio'] = []
            self.metrics_record['val_maskpsnr_psnr_event'] = []
            self.metrics_record['val_maskpsnr_psnr_non_event'] = []
    ...
    def _print_val_log(self):
        ...
        for k in self.validation_metrics.keys():
            if k=="maskpsnr":
                print_content += f'{k}_mask_ratio:{np.mean(self.metrics_record[f"val_{k}_mask_ratio"]):.4f}\t'
                print_content += f'{k}_psnr_event:{np.mean(self.metrics_record[f"val_{k}_psnr_event"]):.4f}\t'
                print_content += f'{k}_psnr_non_event:{np.mean(self.metrics_record[f"val_{k}_psnr_non_event"]):.4f}\t'
            else:
    ...
    def update_training_metrics(self, res, gt, epoch, step, lr, *args, **kwargs):
        ...
        if step % self.train_print_freq == 0:
            ...
            if step == 0:
                self.metrics_record['training_time'].pop(0)

        for k in self.training_metrics.keys():
            func, as_loss = self.training_metrics[k]
            if k == 'MaskCharbonier':
                loss_item = func.forward(res, gt, kwargs['mask'])
            elif k == 'ECharbonier':
                events =  args[0]
    ...
    def update_validation_metrics(self, res, gt, epoch, data_in, *args, **kwargs):
        ...
        for n in range(res.shape[1]):
            ...
            for k in self.validation_metrics.keys():
                if k == 'maskpsnr':
                    val_dict = self.validation_metrics[k].forward(res[:, n].detach(), gt[:, n].detach(), data_in['event_mask'][:, n:n+1].cuda().detach())
                    for subk, subval in val_dict.items():
                        if subval is None:
                            continue
                        else:
                            subval = subval.item()
                            self.metrics_record[f'val_{k}_{subk}'].append(subval)
                            detailed_record += f'{k}_{subk}: {subval:.4f}\t'
                else:
