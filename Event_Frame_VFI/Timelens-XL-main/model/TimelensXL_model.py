...
class Expv8_large(BaseModel):
    def __init__(self, params):
        ...
        self.net = FinalBidirectionAttenfusion(**params.model_config.define_model)
    ...
    def net_training(self, data_in, optim, epoch, step):
        ...
        loss_weight = data_in['loss_weight'].cuda()
        ...
        recon = res[0]
        if 'event_mask' in data_in.keys():
            mask = data_in['event_mask'].cuda()
            loss = self.update_training_metrics(recon, gts, epoch, step, optim.param_groups[0]['lr'], events,
                                                fuseout=res[-1], mask=mask) * loss_weight
        else:
            loss = self.update_training_metrics(recon, gts, epoch, step, optim.param_groups[0]['lr'], events,
                                                fuseout=res[-1]) * loss_weight
        if torch.isnan(loss):
