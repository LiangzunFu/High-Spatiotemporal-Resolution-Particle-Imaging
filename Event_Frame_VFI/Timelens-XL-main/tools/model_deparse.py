...
from model.TimelensXL_model import Expv8_large
...
def deparse_weights(weights_path):
    ...
    return weights, epoch, metrics, scheduler_dict


def save_model(epoch, metrics, params, model, scheduler):
...
def deparse_model(params):
    ...
    model = Expv8_large(params).cuda()

    if params.model_config.model_pretrained is not None:
        weights, epoch, metrics, scheduler_dict = deparse_weights(params.model_config.model_pretrained)
        if params.enable_training:
            epoch += 1
        model.load_state_dict(weights_partial_loading(model, weights))
        # model.load_state_dict(weights)
        print(
            f'Weights successfully load from {params.model_config.model_pretrained}\nAfter loading, the epoch is: {epoch}')
    else:
        epoch = 0
        metrics = {}
        scheduler_dict = None
    ...
    return model, epoch, metrics, scheduler_dict
